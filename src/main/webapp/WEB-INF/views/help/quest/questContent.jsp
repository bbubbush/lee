				...
				<form action="questReplyWrite.ju">
					<table class="table table-condensed">
						<tr>
							<td><div class="col-md-8">
									<strong> 제목 : ${dto.qu_subject}</strong>
								</div>
					...
					<table class="table table-striped table table-hover">
						...
						<c:if test="${!empty next }">
							<tr>
								<td colspan="3"><a
									href="questContent.ju?qu_idx=${next.qu_idx}">△${next.qu_subject}</a></td>
								<td>${next.mem_idx}</td>
							</tr>
						</c:if>
						<c:if test="${!empty pre }">
							<tr>
								<td colspan="3"><a
									href="questContent.ju?qu_idx=${pre.qu_idx}">▽${pre.qu_subject}</a></td>
								<td>${pre.mem_idx}</td>
							</tr>
						</c:if>
					</table>
					<input type="hidden" name="reply_cate" value="${dto.qu_idx}" /> <input
						type="hidden" name="mem_idx" value="${sessionScope.sidx}" />
				</form>
			</div>
		</div>
	</div>
