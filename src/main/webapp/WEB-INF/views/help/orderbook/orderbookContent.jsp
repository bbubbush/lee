			...
			<div class="col-md-11">
				<form action="orderbookReplyWrite.ju">
					<table class="table table-condensed">
						<tr>
							<td><div class="col-md-8">제목 :
									${dto.orderbook_subject}</div>
							...
							<td>
								<div class="row">
									...
									<c:if test="${chk}">
										<div class="col-md-2">
											<a
												href="orderbookChange.ju?orderbook_idx=${dto.orderbook_idx}">글
												수정</a>
										</div>
										<div class="col-md-2">
											<a
												href="orderbookDelete.ju?orderbook_idx=${dto.orderbook_idx}">글
												삭제</a>
										</div>
									</c:if>
								</div>
					...
					<table class="table table-striped table table-hover">
						...
						<c:if test="${!empty next }">
							<tr>
								<td colspan="3"><a
									href="orderbookContent.ju?orderbook_idx=${next.orderbook_idx}">△${next.orderbook_subject}</a></td>
								<td>${next.mem_idx}</td>
							</tr>
						</c:if>
						<c:if test="${!empty pre }">
							<tr>
								<td colspan="3"><a
									href="orderbookContent.ju?orderbook_idx=${pre.orderbook_idx}">▽${pre.orderbook_subject}</a></td>
								<td>${pre.mem_idx}</td>
							</tr>
						</c:if>
					</table>
